...
def main():
    global screen, algo_is_showing, dragging_for_walls
    ...
    b.all_squares(instant=False, color=BLUEISH_COLOR)
    dragging, start, end = False, True, True
    placing, algo_is_showing, dragging_for_walls = False, False, False
    pygame.mouse.set_cursor(pygame.cursors.Cursor(pygame.SYSTEM_CURSOR_ARROW))
    color, bit = BLUEISH_COLOR, 1
    hashMap = {0: GREY_COLOR, 1: BLUEISH_COLOR}
    while True:
        b.all_squares(instant=True, color=color)
        screen.fill(WHITE)
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                pygame.quit()
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE:
                    if not algo_is_showing and not placing:
                        if chosen == 0:
                            b.dfs_graphics()
                        elif chosen == 1:
                            b.bfs_graphics()
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if event.button == 2:
                    ...
                    if yes and event.button == 1:
                        b.grid[row_num][col_num].mark_cell(-2)
                    elif yes and event.button == 3:
